...
* @Last Modified time: 2017-06-02 11:49:34
*/
...
class proc {
	...
	/**
	 * [__unprepare unprepare statements]
	 * @param  {[Object]} dataRequest [description]
	 * @return {[Object]}             [description]
	 */
	__unprepare(dataRequest) {
		dataRequest.unprepare((err) => {
			if (err) {
				throw new Error('Unable to unprepare sql query');
			}
		});
	}

	executeQuery(type, params) {
		const self = this;
		return new Promise((success, fail) => {
			try {
				if (!self.connection) {
					throw new Error('Unable to connect to SQL Server using provided config');
				}

					// Initialize request first
				const dataRequest = new sql.PreparedStatement(self.connection);
				...
				self.sqlParams.input
					.map(data => {
						if (empty(params[data.fieldname]) && data.isNull === true) {
							dataRequest.input(data.fieldname, sql[data.datatype](data.length));
							values[data.fieldname] = params[data.fieldname];
						} else if (empty(params[data.fieldname]) && data.isNull === false) {
							self.errorResponse.push(data.fieldname + ' is invalid . Passed value: ' + params[data.fieldname]);
						} else {
							const check = self.mapdatatype(data.fieldname, data.datatype, params[data.fieldname]);
				...
				dataRequest.prepare(self.sqlParams.query[type], (err) => {
					const result = [];
					// If error occurs
					if (err) {
					...
					// if we get any kind of sql error fail back with the error
					if (dataRequest.parameters.errorMsg.value.length) {
						throw new Error(dataRequest.parameters.errorMsg || 'Unknown sql error occured');
					}

					dataRequest.execute(values, (err, recordset) => {
						if (err) {
							// Cleanup
							self.__unprepare(dataRequest);
							throw new Error('Unable to execure sql query');
						}

						// check all the records returned from sql server
						if (recordset && recordset[0]) {
							if (recordset[0].length > 0) {
								recordset[0].forEach((data) => {
									result.push(data);
								});
							}
						}
						if (self.sqlParams.output) {
							self.sqlParams.output.forEach((data) => {
				...
				return fail(errs.merge(err, {
					title: 'sqlExecutionError',
					message: err.message || 'Unable to execute sql query',
					parameters: self
				}));
			}
		}); // END PROMISES
